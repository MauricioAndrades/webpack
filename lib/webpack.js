...
function webpack(options, callback) {
	...
	if (compiler && compiler.options && Array.isArray(compiler.options.plugins)) {
        compiler.options.plugins = (() => {
            let hasHotModuleReplacementPlugin = false;
            return compiler.options.plugins.reduce((plugins, plugin) => {
                const pluginProtoName = Object.getPrototypeOf(plugin).constructor.name;
                if (pluginProtoName === "HotModuleReplacementPlugin") {
                    if (!hasHotModuleReplacementPlugin) {
                        hasHotModuleReplacementPlugin = true;
                        plugins.push(plugin);
                    }
                } else {
                    plugins.push(plugin);
